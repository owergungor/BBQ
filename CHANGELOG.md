...
- **Production Packaging & Bundle Infrastructure**:
  - Full Tauri 2 bundle configuration with per-user (`currentUser`) NSIS installer for Windows.
...
### Platform Support
- **Windows**: Full native support for Windows 10/11 x64, including WinRT media transport, Win32 global hotkeys, per-user NSIS installer, and registry autostart.
- **macOS**: Bundle configuration for `.app` and `.dmg` targets with Cocoa windowing, LaunchAgents autostart, and MPNowPlaying integration.
- **Linux**: Bundle configuration for AppImage and deb packages with X11/Wayland fallbacks, MPRIS media controls, and XDG autostart.

### Known Limitations
- **Linux Wayland**: Compositor-specific restrictions on global hotkey registration and absolute window placement on certain Wayland compositors (gracefully falls back to in-app shortcuts and standard window mode).
